...
def _resolve_embedding_bag_mean_backend(
	...
	if backend == "auto":
		if _embedding_bag_mean_kernel is not None:
			return "torch"
		# Backward uses atomics and is shape-sensitive; keep automatic TileLang for inference paths.
		if torch.is_grad_enabled() and embedding_weight.requires_grad:
			return "torch"
		if (
			tilelang_available()
			and embedding_weight.device.type == "cuda"
			and embedding_weight.dtype in {torch.float16, torch.bfloat16, torch.float32}
			and values.dtype in {torch.int32, torch.int64}
		):
			return "tilelang"
		return "torch"
	if backend == "torch":
...
def resolved_embedding_bag_mean_backend(
	embedding_weight: torch.Tensor,
	values: torch.Tensor,
	backend: EmbeddingBagMeanBackend = "auto",
	*,
